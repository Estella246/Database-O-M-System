...
- **历史回填**：工具栏「回填」按钮（须 `major_problem_create` 非 hidden）循环调用 `POST /api/major-issues/backfill`，按快照 `event_level` 游标分批 upsert 到 `major_issue`；CLI `python scripts/backfill_major_issue.py` 等同全量跑完
