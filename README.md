...
- **开发分析 · 协同处理人**支持**多选**：节点字段 `ui_props.multiple=true`（迁移 `0053_dev_analysis_collaborator_multiple.sql`）；前端扁平下拉可勾选多人并以全角分号 `；` 拼接落库（每人仍为「姓名 账号」）；运维闭环继承该字段时按同一字符串展示。
...
- RBAC 权限管理系统
- 值班日历与轮值表管理
- 请假申请功能（列表支持分页：每页 10/20/50/100 条；审批「同意申请」后，申请人将在全部轮值表与局点值班表中自动置灰，工单自动派单时跳过该人员；全部时间段结束后自动恢复为当值；内核/管控值班表、RL 值班表不受影响）
