...
- **统计图表**：工单度量 Tab 新增「工单数量趋势」（仅 **全量问题** 一条折线，数据同 `trend.total`）；「按版本透视问题数量」与「R版本透视问题数量」合并为「版本工单数量趋势」，卡片内「版本粒度」下拉切换 **B版本**（原按版本透视，完整 `gauss_version`）、**C版本**（`VxxxRxxxCxx` 如 V500R001C00，或 `x.y.z` 两个小数点如 505.2.1；更长串归到该 C）、**R版本**（原 R 线透视）；C 粒度由已有 `by_version` 归并，无需回填日汇总。工单度量新增「工单数量TOP版本」柱状图（与趋势图共用「版本粒度」，「显示条数」5/10/15/20），取代原「全量问题TOP版本」
