...
- 老库节点别名：更老流程首节点「HCS人员填写」「BU人员填写」（`node_id=8`）与标准「问题填写」同义，迁入时映射为 `problem_fill`；其它别名见 `LEGACY_NODE_NAME_TO_KEY`（如「运维人员分析」→运维分析）。**歧义**：`status=问题审核关闭` 时实例 `current_work_flow_node_name` 或末条 task 的 `next_work_flow_node_name` 常被误存为「问题审核」，迁入/重建流转会按 **审核关闭**（`audit_close`）处理，而非 `problem_review`；task 上 `next_work_flow_node_id=7` 优先于节点名
