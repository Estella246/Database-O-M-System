...
- **详情页签**：点击卡片不再弹窗，而是在顶部工作区打开可关闭小页签（标签为 `item_no`），机制与工作台点开问题单一致；深链 `/tool-plaza/{item_no}`（如 `/tool-plaza/SKILL20260701000`），详情页提供「分享链接」；页签正文优先展示发布时填写的 **详情**（Markdown 渲染），并保留「使用方式」等区块
- **Skill**：发布时上传包含 `SKILL.md` 的文件夹 `.zip`；须填写**详情**与**使用方式**（均为 Markdown，发布弹窗内实时预览）；详情页分别展示「详情」「使用方式」与 `SKILL.md` 渲染结果
- **工具**：发布时上传 `.zip` 工具包，并填写 Markdown **详情**与**使用方式**；卡片摘要与详情页签展示详情内容
