...
- 老库节点别名：更老流程首节点「HCS人员填写」「BU人员填写」（`node_id=8`）与标准「问题填写」同义，迁入时映射为 `problem_fill`；其它别名见 `LEGACY_NODE_NAME_TO_KEY`（如「运维人员分析」→运维分析、「开发人员分析」→开发分析、「开发人员闭环」→开发闭环）。**歧义**：`status` 含「审核关闭」或末条 task 已在「运维闭环」时，实例 `current_work_flow_node_name` / task `next_work_flow_node_name` 误存「问题审核」会按 **审核关闭**（`audit_close`）处理；**开发分析及之后阶段**的 task `next` 若误存「问题审核」（含 `next_work_flow_node_id=2` 与节点名冲突），迁入与 **重建流转** 共用同一套纠正逻辑，按主路径映射为开发闭环/运维闭环/审核关闭等，不再落成「开发分析→问题审核」；task 上 `next_work_flow_node_id` 与节点名冲突时，对 **next** 节点在 id 误为问题审核时会优先采用合法的后续节点名
...
- 权限策略「值班表 / 编辑按钮、下载模版、导入按钮」白名单新增 **仅展示RL值班表相关编辑按钮**：仅 RL 值班表显示编辑能力，月历/轮值/节假日等编辑与导入仍受「展示」策略控制；后端 `PUT /api/duty/rl-oncall` 在 editable 级别下仍允许写入
- 权限策略「值班表」白名单 **仅展示RL值班表**：侧栏仍可进入值班表页，但页面与子菜单仅展示 RL 值班表区块；若同时配置编辑为「展示」，会自动级联为「仅展示RL值班表相关编辑按钮」
...
- 工作台/补丁管理「创建」弹窗：本地预分配工单号尚未落库时，`GET /api/tickets/{id}/nodes/{key}/data` 返回 404 `ticket not found`；前端建单草稿流现将其视为空数据并正常展示表单（不再误报为无法连接后端）。**创建弹窗仅拉取 schema、不拉节点 data**，避免详情页已浏览工单的表单缓存或本地工单号与库内已有单号碰撞时把旧单内容带入新单。
...
- 创建弹窗本地单号与库内已有单碰撞时误挂旧单（如运维分析先起单、TAC 再从问题填写提交到同号）：创建弹窗打开时改调 `POST /api/tickets/allocate-no` 服务端取号（`ticket_global_seq` 维护全局序号 a，000–999 跨日连续、仅 999 后回 000；单号=当天日期+a，已存在则 a+=1 重试）；创建弹窗首次提交带 `create_intent`，单号已被占用时同样按 a 重新取号建单（`backend/utils/ticket_no.py`、`backend/routers/tickets.py`）。误挂数据回滚见 `scripts/sql/rollback_misattached_problem_fill.sql`；须执行迁移 `0089_ticket_global_seq.sql`。回归见 `test/test_ticket_create_collision.py`、`test/test_ticket_no_format.py`
