...
- **惰性同步**：列表接口每次拉取前在数据库侧**增量** upsert（每批最多 500 条待同步工单，不将全量工单载入应用内存）；工单提交（运维分析 / 开发分析 / 问题填写 / 运维闭环等节点）时亦会单条同步。快照字段（局点 / 级别 / 描述 / 运维分析人 / 开发分析人 / 通报日期）随之刷新，进展记录不受同步影响；已生成的问题行即使工单级别后续变化也不自动删除。接口失败时列表页展示错误原因（如表未迁移），不再静默显示「暂无数据」
