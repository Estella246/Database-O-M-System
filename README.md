...
- 支持发起人选择或规则自动带出
- 白名单机制控制可选处理人范围
- **运维分析**处理方式为「提交运维闭环」时，「下一步处理人」默认带出**该工单当前处理人**（可手改；未填时后端提交亦按此默认）；其他处理方式不变
- **开发闭环**处理方式为「提交运维闭环」或「返回运维分析」时，「下一步处理人」默认带出**运维分析阶段最后一次提交人**（可手改；未填时后端提交亦按此默认）
...
- 搜索：编号 / 分类 / 代表问题 / 领域 / 模块&特性 / 问题描述 / 改进诉求 / 提出人 / 接纳状态 / 优先级 / 计划版本（防抖 400ms；立即写 state、中文 composition、Enter 立即搜索、拉数后恢复焦点）
- 筛选与范围：支持「全部 / 我提出的」范围，分类 / 优先级 / 接纳状态多值筛选；分页
- 分析看板（📊 分析）：改进项总数 / 进行中 / 超时 KPI，阶段分布、改进类型分布、**领域分布 / 模块&特性分布**（柱状图带数值标签，悬停浮动提示）、**领域 × 用户矩阵**（提交数 / 接纳数，可按领域筛选，选中领域时表头显示「小计」）、耗时 Top（当前滞留）。时间预设切换或自定义日期时会重置领域筛选
- 操作日志：记录创建 / 编辑 / 状态变更
...
- 工单搜索功能
  - 在工作台搜索框输入关键词，实时搜索工单
  - 搜索匹配全部文本字段（约91个字段，包含所有节点数据与各阶段处理人）
  - 支持搜索工单号、标题、处理人、描述、局点、问题阶段、DTS单号等
  - 搜索结果自动适配权限策略（仅显示用户可见的工单）
  - 中文输入支持（防抖与停手重绘均为 500ms，Enter 立即搜索；拉数前不整页重绘；**输入/拼音期间挂起整页刷新，停手后再刷**；拉数结束成功重绘会清除挂起债务，避免同一批结果连刷两次；拉数过程中保留上一页列表，避免刷新窗口空白；全站列表搜索框统一交互原则）
- SLA 时间计算：列表以 `ticket.created_at` 为起点；终点为当前时间或 `closedAt`；挂起区间不计入（`slaPausedSeconds` + 当前挂起段）。例：12:00 建单 → 13:00 挂起 → 14:00 解除 → 15:00 关单 = 2 小时
- 表格列选择功能
  - 点击「选择列」按钮可自定义表格展示列
  - 支持选择各流程阶段的文本字段（约91个可选；含问题审核→审核关闭各阶段「处理人」；流转控件「下一步处理人」不进列选/导出，仅问题审核保留「处理方式」）
...
- 数据导出功能
  ...
  - 单元格统一压成单行（换行/空白折叠为空格）并截断至约 2000 字，避免 CSV 中长字段（报错信息、Core 堆栈等）另起行导致物理行数多于工单数+表头
...
- 富文本编辑：整体情况 4 段、重大问题/改进诉求表格单元格、问题详情自由文本编辑区均支持**加粗 + 预设字体颜色**（工具条 `frontend/modules/ui/rich-text.js`，execCommand 实现，DOMPurify 清洗，仅允许 b/strong/i/em/u/span/font/br/div/p 与 color/font-weight 样式）；存储为清洗后的 HTML。网页只读与 **HTML 导出**输出同一份 HTML，字号/加粗/颜色一致；**Excel 导出**经 `richToPlainText` 退化为纯文本
- 五段结构（均按段保存）：
  ...
  - 四、改进诉求：3 个图表 + 本月质量改进记录表（关联工单 / QI编号 / 改进标题 / 分类 / 领域 / 提出人）；段头「导入」从 qi_request 拉取；关联工单、QI编号可跳转问题详情与质量改进单；SQL/存储领域改进默认 Top10
  - 五、问题详情&质量改进记录：自由文本段（富文本编辑），不支持导入
- 段头样式：天蓝色横条
- 从本月工单/质量改进导入（问题透视 / 重大问题 / 改进诉求各段头「编辑本段」左侧的「导入」按钮；归档态隐藏）：按所选报告月份聚合计算后填入草稿，进入编辑态供核对，再「保存本段」。归月口径 = 工单/QI `created_at`（Asia/Shanghai 自然月，与工作台顶栏日期筛选一致）；**内核质量问题** = 问题组件（`component`）为「内核问题」且 是否质量问题（`is_quality_issue`）∈{是（已知质量问题）, 是（新发现质量问题）}；数据源 **`ticket_list_snapshot.extra_fields`**（与工作台列筛选同源，`is_quality_issue` 取快照当前值、无粘性）。须快照已就绪（迁移 0079 + 重建列表快照）。磐石版本相关暂不计算（KPI 的 `pansh_*` 保留页面手填值）
  ...
  - 改进诉求（`GET /api/monthly-report/{ym}/import/improve`）：数据来自「质量改进」(`qi_request`，排除草稿)。**领域占比 / SQL 领域改进 / 存储领域改进**取**全部**非草稿质量改进（不限月份）——领域占比按 `domain` 分组计数，SQL/存储分别取 domain 含「SQL」/「存储」的项按 `module_feature` 分组计数并默认取 **Top10**；**本月质量改进记录**表仅取 `created_at`（Asia/Shanghai）落在所选月份的项，映射 关联工单←related_ticket_no / QI编号←qi_no / 改进标题←title / 分类←category / 领域←domain / 提出人←proposer（附 `_qi_id` 供跳转）；领域占比饼图为左右结构（左饼右竖排可滚动图例），扇区标签仅显示占比
...
- 工单流转通知：HCS工单流转到「问题审核」「运维分析」「开发分析」节点时，自动向该节点处理人推送小鲁班通知消息（包含工单号、当前节点、起始日期、严重性、局点、问题组件、eCare单号、问题描述、工单链接）；问题描述超长时截取前100字符；正向流转、回退、重分配均触发通知；**例外**：问题审核选择「确认问题」提交时，不向下一处理人（本人）发私信，仅发群通知；通知失败仅打印error日志，不影响工单主流程；工单链接为完整 URL：`{链接前缀}/tickets/{工单号}`，链接前缀优先取 `APP_PUBLIC_BASE_URL`，未配置时默认 `https://gaussdb-ops.rnd.huawei.com`
- 问题审核群通知：工单在「问题审核」节点选择「确认问题」提交后，向指定群推送通知（含流程ID、起始日期、局点、问题阶段、产品线、问题严重性、问题组件、eCare单号、问题描述、**运维人员**（末行，审核阶段当前处理人）；问题描述超长时截取前100字符）；群号通过 `XIAOLUBAN_GROUP_CHAT_ID` 配置；此时不再向处理人发个人小鲁班消息
- 运维闭环提单人通知：HCS 工单处理方式为「提交运维闭环」（运维分析或开发闭环）到达「运维闭环」时，向**提单人**（问题填写 `hcs_owner`，无法解析账号时回退工单 `creator_id`）推送小鲁班私信：「此问题已闭环，请提单人尽快与运维侧对齐，如实填写改进建议」及完整问题链接 `{链接前缀}/tickets/{工单号}`；通知失败仅打印 error 日志，不影响工单主流程
...
- 关键词搜索：匹配局点名称、地区、代表处、运维人员、客户/项目/服务经理、风险描述等文本字段（防抖 800ms，与工作台搜索原则一致：立即写 state、中文 composition、Enter 立即搜索、拉数后恢复焦点），支持分页（每页 10/20/50/100 条）
...
- **历史回填**：页面不展示「回填」按钮；运维可通过 `POST /api/major-issues/backfill`（须 `major_problem_create` 非 hidden）或 CLI `python scripts/backfill_major_issue.py` 从 `ticket_list_snapshot.extra_fields.event_level` 筛出命中阈值的 `ticket_id` 后游标逐条 upsert；**已在 `major_issue` 的命中工单快速跳过**（单次请求最多连跳 50 条）；与工作台列筛选同源表达式；请求带 `X-Stream-Keepalive: 1` 防网关 504
...
- **列表勾选与导出**：与工作台一致支持行勾选与本页全选（跨页保留选中）；工具栏「导出」按钮（权限项 `major_problem_export`）导出 Excel（`.xlsx`）单 sheet「重大问题」——基础字段后为「进展内容」「消减措施」「记录人」；进展时间分别拼进进展内容与消减措施单元格（无单独进展时间列）；按天多行，有勾选时导出选中项，无勾选时导出当前筛选条件下全部数据
...
- **工具**：发布时上传 `.zip` 工具包（单文件最大 **100MB**），并填写 Markdown **详情**与**使用方式**；卡片摘要与详情页签展示详情内容
- **标签**：发布时手填或从已有标签下拉选择（`datalist`，来源于已发布资源的去重标签）
...
- 重建粒度：依据 `t_work_flow_task` 流转记录**逐节点重建** `ticket_node_instance` / `ticket_node_data` / `ticket_flow_log`，字段值取自 `t_work_flow_task_parse`（`column1..column64`）与 **`t_work_flow_task.form_data`**（按 `cnFieldName` 映射，富文本含图片时**优先 form_data**，因 parse 列 `VARCHAR(2000)` 会截断），并按新平台 `node_field_def` 的归属节点落位；当前节点/处理人/状态由 `t_work_flow_instance` 决定（`进行中→open`、`暂停→suspended`、`关闭/完成/非问题关闭→closed`）
...
- 接口：`POST /api/tickets/migrate-legacy`，可选请求体 `process_ids`（流程 ID 数组，仅迁入指定工单）；不传则迁入全部。**迁入全部**时前端默认每批 `max_total=10`、`after_legacy_instance_id` 游标续跑；**不自动重建列表快照**，须在工作台顶栏手动点 **重建列表快照**（`POST /api/tickets/snapshot/rebuild`）。接口仍支持 `refresh_snapshot: true`（`max_total=0` 时仅重建快照）。后端未传 `max_total` 时默认 cap 为 `batch_size`（`MIGRATE_LEGACY_DEFAULT_CAP_BATCH=1`）。前端单批最长等待 **5 分钟**；日志含 `migrate_legacy batch fetched`、`migrate_legacy instance start/ok/failed`、`migrate_legacy mem rss_mb=…`。返回 `{ ok, migrated, …, ticket_nos_count, has_more, next_after_legacy_instance_id }`（不含 `ticket_nos` 数组）
...
- 自动化测试：`test/test_m12_legacy_migration.py`、`test/test_legacy_form_data.py`。测试夹具用低位 id（`1001-1004`）以「建表 `IF NOT EXISTS` + `INSERT`」方式灌入后端实际所读老库（不 DROP 表，保护演示库的 2000 条），迁移调用传 `batch_size=4`/`max_total=4`，按 `id` 升序只处理最低的 4 条夹具单，与演示数据互不干扰。
...
| 工作台 | `/workbench` | 工单列表、创建、导出（HCS_INCIDENT，不含热补丁单）；**列表读 `ticket_list_snapshot` 快照表**：服务端分页、全量搜索/页签/列筛选在 SQL 层完成，仅返回当前页；列筛选 ⏷ 支持 **whitelist（下拉）**、默认可筛列及选择列中的**各阶段处理人**（**不含起始日期、问题描述、richtext**）；可选值走 `GET /api/tickets/facets`（全量 distinct） |
...
```json
{
  ...
  "meta": {
    "suggested_next_handler_by_handle_mode": {
      ...
      "返回运维分析": "运维最后提交人 ops_last_submitter"
    }
  }
}
```

- `meta.suggested_next_handler_by_handle_mode`：仅 **开发闭环** 等需要按处理方式默认带出下一步处理人的节点会返回；前端选中「提交运维闭环」或「返回运维分析」时预填「下一步处理人」。
...
项目在 `test/` 目录下提供完整的功能测试方案与自动化测试程序，覆盖全部12个功能模块共490+个测试用例，以及200+个E2E端到端测试用例。
...
| `test/e2e/test_e2e_qi_config.py` | 10 | 质量改进配置页（迁移按钮/白名单全选/搜索回车）+ 统计分析页（页面加载/分布区块/领域×用户矩阵领域筛选/切换预设重置筛选/柱图数值标签/空proposer兜底「未知」） |

> 上表仅列代表性文件；完整覆盖见 `test/e2e/` 目录（30+ 个文件、300+ 个用例），故总数大于表格合计。
...
**工单流转**
- **开发闭环「提交运维闭环」/「返回运维分析」**：下一步处理人默认带出运维分析阶段最后一次提交人（可手改；未填时后端提交亦按此默认）。单测见 `test_m02_ticket.py::test_e_m02_dev_closure_to_ops_closure_defaults_next_handler_to_ops_submitter`、`test_e_m02_dev_closure_return_ops_defaults_next_handler_to_ops_submitter`
...
- **问题填写「局点」下拉仅可选局点档案**：「局点」字段为可搜索扁平下拉，选项实时取自「局点档案」的局点名称（`LOCATION_SET` 选项集走 `external_api`，后端 `_load_schema` 按 `site_profile.site_name` 填充）；**禁止手动输入新局点**，未录入局点须联系周晨雷 30036630 维护局点档案后再选。已部署库请执行 `db/migrations/0054_problem_fill_location_site_profile.sql`
...
- **用户管理搜索框**：管理 → 用户管理页表格上方提供全局搜索，可按账号、姓名、角色、小组、邮箱、联系电话、产品线、领域、最小部门、备注等字段关键词过滤，与表头列筛选、分页可同时使用；输入防抖 800ms 后刷新列表，Enter 立即搜索，重绘后恢复搜索框焦点
...
- **问题填写组件联动**：「产品线」=`公有云` 时，「问题组件」仅可选「内核问题」（前端扁平下拉联动，隐藏非法选项并在取值冲突时清空；无默认值，须用户手动选择；后端提交校验兜底）
...
**功能更新**
- **启动/重启日志横幅**：服务就绪时打印 ASCII 图案（`backend/utils/startup_banner.py`）；`scripts/start.py` 开场横幅同步
...
- **运维闭环 · 是否输出问题报告**：新增必填下拉「是否输出问题报告」（是/否）；选「是」时展示并必填「上传问题报告」（`file` 类型，MinIO 上传）。已部署库请执行 `db/migrations/0091_ops_closure_output_problem_report.sql`
...
- **统计图表**：工单度量 Tab 新增「工单数量趋势」（仅 **全量问题** 一条折线，数据同 `trend.total`）；「按版本透视问题数量」与「R版本透视问题数量」合并为「版本工单数量趋势」，卡片内「版本粒度」下拉切换 **B版本**（原按版本透视，完整 `gauss_version`）、**C版本**（`VxxxRxxxCxx` 如 V500R001C00，或 `x.y.z` 两个小数点如 505.2.1；更长串归到该 C）、**R版本**（原 R 线透视）；C 粒度由已有 `by_version` 归并，无需回填日汇总。工单度量新增「工单数量TOP版本」柱状图（独立「版本粒度」B/C/R +「显示条数」5/10/15/20），取代原「全量问题TOP版本」
- **统计图表**：工单度量 Tab 新增「质量问题数量趋势」（与「工单数量趋势」同时间轴与折线样式，数据同 `trend.quality_yes`）：在全量工单上按「是否质量问题」过滤，取值不是「否」的计入（即「是（已知质量问题）」「是（新发现质量问题）」；未填不计入）
- **统计图表**：工单度量 Tab 新增「质量问题版本趋势」（与「版本工单数量趋势」同 B/C/R 粒度下拉与折线样式，不改动原图）：按「是否质量问题」过滤，取值不是「否」的计入（已知/新发现；未填不计入）；数据为 `by_version_time_quality` / `by_c_version_time_quality` / `by_r_version_time_quality`
- **统计图表**：工单度量 Tab 新增「质量问题TOP版本」（与「工单数量TOP版本」同 B/C/R 粒度 + 显示条数，不改动原图）：按「是否质量问题」过滤，「是（已知质量问题）」「是（新发现质量问题）」计入，否与未填不计入；数据由 `by_*_version_time_quality` 汇总 TOP N
...
- **统计图表**：问题归属 Tab「按版本透视问题数量」「版本问题类别走势」均收录当前筛选时间窗内出现过的**全部有效版本**（仍排除「未知版本」与数量为 0 的版本），不再截断为 Top11
- **统计图表**：问题归属 Tab「按版本透视」悬停 tooltip 仅列该时间点数量 > 0 的版本并按数量降序；另设 `confine` / `appendToBody` / `enterable` 与限高可滚动，悬停时滚轮优先滚动 tooltip（避免被 dataZoom 抢走）
...
- **统计图表**：问题归属 Tab「R版本透视问题数量」纳入 **507**；`_r_of_version` 从完整内核版本串归类（如 `GaussDB Kernel 506.x`→506），不再用 `startswith`（否则全落到默认 505、503/506 为 0）；无法识别的版本不计入；历史日汇总须 **回填日汇总** 后生效
...
- **统计图表 · 人力投入**：人员轴柱状图（人力投入统计、未闭环滞留人、各阶段人员滞留、问题流转详细占比）卡片区默认只展示前 **15** 人；点放大后仍展示全部人员
