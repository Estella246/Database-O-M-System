...
- 操作日志：记录创建 / 编辑 / 状态变更
- 导出：点击「导出」导出全部为 Excel，列与表格一致；权限项 `requirement_export` 控制按钮显示（默认隐藏，需白名单授予）
- 导入：点击「下载模板」获取 Excel 模板（表头按上述列名匹配，第 3 行起为数据），填写后「导入」批量入库；非法枚举（分类/优先级/接纳状态）自动回落默认值；权限项 `requirement_import` 控制按钮显示
