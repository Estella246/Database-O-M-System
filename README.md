...
- **工具**：发布时上传 `.zip` 工具包，并填写 Markdown **使用方式**；卡片与详情展示使用说明摘要/全文
...
- 后端：`db/migrations/0097_ops_tool_plaza.sql`、`0098_ops_tool_usage_md.sql` + `backend/routers/ops_tool_plaza.py`（`/api/ops-tool-plaza`）；前端 `frontend/modules/pages/tool-plaza-page.js`
...
- 接口：`POST /api/tickets/migrate-legacy`，可选请求体 `process_ids`（流程 ID 数组，仅迁入指定工单）；不传则迁入全部。**迁入全部**时前端默认每批 `max_total=100`、`after_legacy_instance_id` 游标续跑，全部完成后单独请求 `refresh_snapshot: true` 重建列表快照（避免单次 HTTP 超时）。前端单批最长等待 **5 分钟**（`MIGRATE_LEGACY_BATCH_TIMEOUT_MS`），请求头 `X-Stream-Keepalive: 1` 时后端每 15 秒输出 keepalive 防止网关 504；生产环境 Nginx 建议 `proxy_read_timeout 300s` 及以上。`GET /api/tickets/migrate-legacy/candidates` 列出老库可选工单（按 `process_id`）。**存量已迁但元数据不对**（流程 ID / status / 当前节点）：迁入弹窗 **修复已迁**，或 `POST /api/tickets/migrate-legacy/repair`（默认仅更新 `ticket_no` / `status` / `current_node_id`）。**流转日志/审核关闭阶段异常**或**富文本缺图/换行丢失**：弹窗 **重建流转**，或同一接口传 `rebuild_workflow: true` 按老库 task 重建节点与 `flow_log`；可选 `process_ids` 仅处理指定单，或 `python scripts/repair_legacy_migrated_tickets.py`。**删除已迁**：迁入弹窗 **删除已迁** / **删除全部已迁**，或 `GET /api/tickets/migrate-legacy/migrated-count` 统计、`POST /api/tickets/migrate-legacy/delete-migrated` 删除（以 `ticket.legacy_instance_id IS NOT NULL` 识别，不可恢复）；命令行 `python scripts/delete_legacy_migrated_tickets.py`（支持 `--dry-run`）。返回 `{ ok, migrated, skipped_existing, skipped_deleted, skipped_not_found, failed, errors, ticket_nos, processed, has_more, next_after_legacy_instance_id }`（repair 返回 `repaired, skipped_unchanged, …`；delete 返回 `deleted, …`）
