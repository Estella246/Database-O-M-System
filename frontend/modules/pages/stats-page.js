...
const STATS_OWNERSHIP_ECHART_IDS = {
  ...
  ownL2ModuleLine: "stats-ownership-echart-l2-module",
...
function isStatsOwnershipVerLineChart(key) {
  return (
    key === "ownVerLine" ||
    key === "ownQualityVerLine" ||
    key === "ownIssueTypeLine" ||
    key === "ownL2ModuleLine"
  );
}
...
function statsOwnershipVersionTrendSeries(byVersionTime, byCVersionTime, byRTime, verGran, showSymbol) {
  ...
  return verGran === "r" ? rSeries : verSeries;
}

function statsOwnershipVersionLineChartOption(lineAnim, ink, commonTooltip, timeLabels, n, series) {
  return {
    ...lineAnim,
    tooltip: {
      ...commonTooltip,
...
export function buildStatsOwnershipChartOptions() {
  ...
  const verLineSeries = statsOwnershipVersionTrendSeries(
    byVersionTime,
    byCVersionTime,
    byRTime,
    verGran,
  ...
  const qualityVerLineSeries = statsOwnershipVersionTrendSeries(
  ...
  const l2ModuleTimeMap = {};
  statsOwnershipTopEntriesFromTimeMap(payload.by_l2_module_time || {}, 10).forEach((e) => {
    l2ModuleTimeMap[e.name] = payload.by_l2_module_time[e.name];
  });
  const l2ModuleLineSeries = statsOwnershipIssueTypeTrendSeries(l2ModuleTimeMap, n < 18);
  ...
  const qualitySitePick = (payload.top_site_quality || []).slice(0, topN).map((x) => x.name);
  ...
  const pieEntriesToSlices = (entries) =>
    (entries || [])
      .filter((x) => x && String(x.name || "").trim() && (Number(x.value) || 0) > 0)
  ...
  const qualitySourcePieSlices = pieEntriesToSlices(payload.quality_source_pie);
  ...
  const options = {
    ...
    ownVerLine: statsOwnershipVersionLineChartOption(
      lineAnim,
      ink,
      commonTooltip,
      scopedLabels,
    ...
    ownQualityVerLine: statsOwnershipVersionLineChartOption(
      lineAnim,
      ink,
      commonTooltip,
      scopedLabels,
    ...
    ownL2ModuleLine: statsOwnershipVersionLineChartOption(
      lineAnim,
      ink,
      commonTooltip,
      timeLabels,
...
export function renderStatsOwnershipSectionCardsHtml() {
  ...
  const topVerGran = statsOwnershipVerGranularity(state.statsOwnershipTopVerGranularity);
  const topVerN = statsOwnershipTopVerN(state.statsOwnershipTopVerN);
  const topVerToolbar = `<label class="stat-labor-filter"><span class="stat-labor-filter-label">版本粒度</span>
      <select class="stat-labor-select" data-stats-ownership-select="statsOwnershipTopVerGranularity">
        <option value="b" ${topVerGran === "b" ? "selected" : ""}>B版本</option>
        <option value="c" ${topVerGran === "c" ? "selected" : ""}>C版本</option>
        <option value="r" ${topVerGran === "r" ? "selected" : ""}>R版本</option>
      </select></label>
    <label class="stat-labor-filter"><span class="stat-labor-filter-label">显示条数</span>
      <select class="stat-labor-select" data-stats-ownership-select="statsOwnershipTopVerN">
        ${STAT_OWNERSHIP_TOP_VER_N_OPTIONS
          .map((n) => `<option value="${n}" ${topVerN === n ? "selected" : ""}>${n}</option>`)
          .join("")}
      </select></label>`;
  ...
  const hL2Module = `<div class="stat-echart-host stat-echart-host--tall" id="stats-ownership-echart-l2-module"></div>${echartsFallback}`;
  ...
  const hQualitySourcePie = `<div class="stat-echart-host stat-echart-host--pie" id="stats-ownership-echart-quality-source-pie"></div>${echartsFallback}`;
  ...
  return [
    ...
    renderOwnershipGlassCard("TOP高发模块问题趋势", "", hL2Module, 5, "ownL2ModuleLine"),
    renderOwnershipGlassCard("工单数量TOP版本", topVerToolbar, hTopVer, 6, "ownTopVer"),
    renderOwnershipGlassCard("质量问题TOP版本", topVerToolbar, hQualityTopVer, 7, "ownQualityTopVer"),
    ...
    renderOwnershipGlassCard("质量问题TOP高发模块", l1Toolbar, hL1, 14, "ownL1Bar"),
  ].join("");
}
...
async function startStatsDailyBackfillFromUi() {
  ...
  try {
    ...
    if (state.statsChartsTab === "doer") invalidateStatsChartsPayload("doer");
    requestRender();
  } catch (e) {
