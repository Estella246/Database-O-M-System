...
BEGIN
  ...
  LOOP
    ...
    SELECT last_suffix INTO a FROM ticket_global_seq WHERE seq_key = v_seq_key FOR UPDATE;
    IF NOT FOUND THEN
      INSERT INTO ticket_global_seq (seq_key, last_suffix) VALUES (v_seq_key, -1);
      a := -1;
    END IF;
    ...
    LOOP
      ...
      guard := guard + 1;
      IF guard >= 1000 THEN
        RAISE EXCEPTION 'ops_tool_item item_no backfill exhausted for %', v_seq_key;
      END IF;
    END LOOP;

    UPDATE ops_tool_item SET item_no = candidate WHERE id = r.id;
    UPDATE ticket_global_seq SET last_suffix = a WHERE seq_key = v_seq_key;
  END LOOP;
END $$;
