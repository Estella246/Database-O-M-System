...
注意：此文件使用 test/e2e/conftest.py 的 fixtures，会自动启动后端服务
"""
...
class TestFrontendUIRendering:
    ...
    def test_tc_m13_025_leave_application_form(self, page, backend_server):
        """TC-M13-025: 请假申请-申请表单"""
        page.goto(f"{backend_server}/#/leave")
        page.wait_for_load_state("networkidle")
...
class TestFrontendEdgeCases:
    ...
    def test_page_not_found_fallback(self, page, backend_server):
        """测试不存在的路由回退到首页"""
        page.goto(f"{backend_server}/#/nonexistent-route")
        page.wait_for_load_state("networkidle")
        layout = page.locator(".layout")
        expect(layout).to_be_visible()

    def test_empty_state_display(self, page, backend_server):
        """测试空状态显示"""
        page.goto(f"{backend_server}/#/leave")
        page.wait_for_load_state("networkidle")
